...
public class KafkaConnLauncher {
    ...
    private static void init() {
        Properties p = null;
        try {
            p = ConfReader.readConf(confPath);
        } catch (IOException e) {
            logger.error("Load config fail:" + confPath, e);
            System.exit(-1);
        }
    ...
    public static void start() {
        ...
        ConfigUpdateMonitor.start(confPath);
    }

    public static void stop() {
        timer.cancel();//cancel并不终止正在执行的任务完成
        ConfigUpdateMonitor.stop();
    }
